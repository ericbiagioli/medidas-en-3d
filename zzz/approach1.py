...
def default_camera_matrix(w: int, h: int):
    ...
    cy = h / 2.0
    K = np.array(
        [[focal_length_px, 0, cx], [0, focal_length_px, cy], [0, 0, 1]],
        dtype=np.float64,
    )
...
def detect_markers(img, marker_side_m: float, K, D, frame_prev=None):
    ...
    for i, marker_id in enumerate(ids.flatten()):
        ...
        ok, rvec, tvec = cv2.solvePnP(
            object_points, image_points, K, D, cv2.SOLVEPNP_IPPE_SQUARE
        )
...
def main():
    ...
    cv2.setWindowProperty(
        params.winname, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN
    )
